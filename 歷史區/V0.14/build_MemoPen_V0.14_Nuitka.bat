...
py -m nuitka ^
  ...
  --remove-output ^
  --output-dir="%CHECK%" ^
  --report="%CHECK%\MemoPen_V0.14_standalone_report.xml" ^
  "%APP%"
if errorlevel 1 goto :error
...
py -m nuitka ^
  ...
  --remove-output ^
  --output-dir="%DIST%" ^
  --output-filename=MemoPen_V0.14.exe ^
  --report="%DIST%\MemoPen_V0.14_onefile_report.xml" ^
  "%APP%"
if errorlevel 1 goto :error

echo.
echo ============================================================
echo 完成：%DIST%\MemoPen_V0.14.exe
echo ============================================================
explorer "%DIST%"
pause
exit /b 0
