...
#    nuitka-project: --product-version=0.14.3.0
#    nuitka-project: --file-version=0.14.3.0
...
class MemoPenWindow(QWidget):
    ...
    def install_resize_filters(self) -> None:
    ...
    def eventFilter(self, watched, event) -> bool:  # noqa: N802 (Qt API)
        ...
        if event_type == QEvent.MouseButtonPress:
            return self.handle_resize_press(watched, event)
        if event_type == QEvent.MouseMove:
            return self.handle_resize_move(watched, event)
        if event_type == QEvent.MouseButtonRelease:
            return self.handle_resize_release(event)
        if event_type == QEvent.Leave and not self._resizing:
            self.unsetCursor()
        return super().eventFilter(watched, event)

    def resize_edges_at(self, position: QPoint) -> Qt.Edge:
        edges = Qt.Edge(0)
    ...
    def update_resize_cursor(self, edges: Qt.Edge) -> None:
        if (edges & Qt.LeftEdge and edges & Qt.TopEdge) or (
            edges & Qt.RightEdge and edges & Qt.BottomEdge
        ):
            self.setCursor(Qt.SizeFDiagCursor)
        elif (edges & Qt.RightEdge and edges & Qt.TopEdge) or (
    ...
    def handle_resize_press(self, watched, event: QMouseEvent) -> bool:
        if event.button() != Qt.LeftButton:
            return False

        position = watched.mapTo(self, event.position().toPoint())
        edges = self.resize_edges_at(position)
        if not edges:
            return False
    ...
    def handle_resize_move(self, watched, event: QMouseEvent) -> bool:
        if self._resizing:
            self.resize_from_mouse(event.globalPosition().toPoint())
            event.accept()
            return True

        position = watched.mapTo(self, event.position().toPoint())
        self.update_resize_cursor(self.resize_edges_at(position))
        return False

    def handle_resize_release(self, event: QMouseEvent) -> bool:
        ...
        self.update_resize_cursor(
            self.resize_edges_at(self.mapFromGlobal(event.globalPosition().toPoint()))
        )
        event.accept()
        return True

    def resize_from_mouse(self, global_position: QPoint) -> None:
        ...
        if self._resize_edges & Qt.LeftEdge:
            geometry.setLeft(
                min(geometry.right() - min_width + 1, geometry.left() + delta.x())
            )
        if self._resize_edges & Qt.RightEdge:
            geometry.setRight(
                max(geometry.left() + min_width - 1, geometry.right() + delta.x())
            )
        if self._resize_edges & Qt.TopEdge:
            geometry.setTop(
                min(geometry.bottom() - min_height + 1, geometry.top() + delta.y())
            )
        if self._resize_edges & Qt.BottomEdge:
            geometry.setBottom(
                max(geometry.top() + min_height - 1, geometry.bottom() + delta.y())
            )
